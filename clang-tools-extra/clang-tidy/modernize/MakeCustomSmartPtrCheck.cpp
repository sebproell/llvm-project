//===--- MakeCustomSmartPtrCheck.cpp - clang-tidy -------------------------===//
//
...
#include "MakeCustomSmartPtrCheck.h"
...
const char MakeCustomSmartPtrCheck::PointerType[] = "pointerType";

MakeCustomSmartPtrCheck::MakeCustomSmartPtrCheck(StringRef Name,
                                                 ClangTidyContext *Context)
    : ClangTidyCheck(Name, Context),
      MakeSmartPtrFunctionName(
          Options.get("MakeSmartPtrFunction", "Teuchos::make_rcp")),
      IgnoreMacros(Options.getLocalOrGlobal("IgnoreMacros", true)),
      IgnoreDefaultInitialization(
          Options.get("IgnoreDefaultInitialization", true)) {}

void MakeCustomSmartPtrCheck::storeOptions(ClangTidyOptions::OptionMap &Opts) {
...
bool MakeCustomSmartPtrCheck::isLanguageVersionSupported(
    const LangOptions &LangOpts) const {
  return LangOpts.CPlusPlus11;
}

void MakeCustomSmartPtrCheck::registerMatchers(
  ...
  auto SmartPointerTypeMatcher = qualType(hasUnqualifiedDesugaredType(
      recordType(hasDeclaration(classTemplateSpecializationDecl(
          hasName("::Teuchos::RCP"), templateArgumentCountIs(1),
  ...
  Finder->addMatcher(
      traverse(
          ...
          cxxBindTemporaryExpr(has(ignoringParenImpCasts(
              cxxConstructExpr(
                  hasType(SmartPointerTypeMatcher), argumentCountIs(2),
                  hasArgument(
  ...
  Finder->addMatcher(
      traverse(TK_AsIs,
               cxxMemberCallExpr(
                   ...
                   anyOf(thisPointerType(SmartPointerTypeMatcher),
                         on(ignoringImplicit(anyOf(
                             hasType(SmartPointerTypeMatcher),
                             hasType(pointsTo(SmartPointerTypeMatcher)))))))
                   .bind(ResetCall)),
      this);
}

void MakeCustomSmartPtrCheck::check(const MatchFinder::MatchResult &Result) {
...
void MakeCustomSmartPtrCheck::checkConstruct(SourceManager &SM, ASTContext *Ctx,
...
void MakeCustomSmartPtrCheck::checkReset(SourceManager &SM, ASTContext *Ctx,
...
bool MakeCustomSmartPtrCheck::replaceNew(DiagnosticBuilder &Diag,
                                         const CXXNewExpr *New,
